...
function Test-JsonFile {
    ...
    Write-Host "Checking Acora $type settings file in $settingsFile (type = $type)"
...
function TestAcoraRepository {
    ...
    # Get-ChildItem needs -force to include folders starting with . (e.x. .github / .Acora) on Linux
    Get-ChildItem -Path $baseFolder -Filter '*.json' -Recurse -Force | ForEach-Object {
        if ($_.Directory.Name -eq '.Acora' -and $_.BaseName -eq 'settings') {
        ...
        elseif ($_.Directory.Name -eq '.github' -and $_.BaseName -like '*ettings') {
            if ($_.BaseName -eq 'Acora-Settings') {
                $type = 'Repo'
            }
            else {
                $type = 'Workflow'
...
function Write-Big {
    ...
    $chars = @{
        ...
        "a" = @(
            "                                        __              _____ _ _   _    _       _       "
            "    /\                                 / _|            / ____(_) | | |  | |     | |      "
            "   /  \    ___    ___    ___   ___ _  | |_ ___  _ __  | |  __ _| |_| |__| |_   _| |__    "
            "  / /\ \  / __|  / _ \  |  _| / _ \ | |  _/ _ \| '__| | | |_ | | __|  __  | | | | '_ \   "
            " / ____ \| |__  | (_) | | |  | |_|  | | || (_) | |    | |__| | | |_| |  | | |_| | |_) |  "
            "/_/    \_\\___|  \___/  |_|   \___/_| |_| \___/|_|     \_____|_|\__|_|  |_|\__,_|_.__/   "
        )
    }
