...
function ModifyUpdateAcoraSystemFiles {
    Param(
        [Yaml] $yaml,
        [hashtable] $repoSettings
    )

    if($repoSettings.Keys -notcontains 'UpdateAcoraSystemFilesEnvironment') {
        # If UpdateAcoraSystemFilesEnvironment is not set, we don't need to do anything
        return
    }

    $updateAcoraSystemFilesEnvironment = $repoSettings.UpdateAcoraSystemFilesEnvironment
    Write-Host "Setting 'Update Acora System Files' environment to $updateAcoraSystemFilesEnvironment"

    # Add or replace the environment: section in the UpdateAcoraSystemFiles job
    $updateAcoraSystemFilesJob = $yaml.Get('jobs:/UpdateAcoraSystemFiles:/')

    if(-not $updateAcoraSystemFilesJob) {
        # Defensively check that the UpdateAcoraSystemFiles job exists
        throw "No UpdateAcoraSystemFiles job found in the workflow"
    }

    $environmentSection = $updateAcoraSystemFilesJob.Get('environment:')
    if($environmentSection) {
        # If the environment: section already exists, replace it with the new environment
        $updateAcoraSystemFilesJob.Replace($environmentSection, "environment: $updateAcoraSystemFilesEnvironment")
    }
    else {
        # If the environment: section does not exist, add it
        $updateAcoraSystemFilesJob.Insert(1, "environment: $updateAcoraSystemFilesEnvironment")
    }

    $yaml.Replace('jobs:/UpdateAcoraSystemFiles:/', $updateAcoraSystemFilesJob.content)
}
