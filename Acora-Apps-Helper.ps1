...
$ALGoFolderName = '.Acora'
$ALGoSettingsFile = Join-Path '.Acora' 'settings.json'
...
# - AcoraOrgSettings (github Variable)                 = Organization settings variable
# - .github/Acora-Settings.json                       = Repository Settings file
# - AcoraRepoSettings (github Variable)                = Repository settings variable
# - <project>/.Acora/settings.json                    = Project settings file
# - .github/<workflowName>.settings.json              = Workflow settings file
# - <project>/.Acora/<workflowName>.settings.json     = Project workflow settings file
# - <project>/.Acora/<userName>.settings.json         = User settings file
function ReadSettings {
    Param(
        ...
        [string] $orgSettingsVariableValue = "$ENV:AcoraOrgSettings",
        [string] $repoSettingsVariableValue = "$ENV:AcoraRepoSettings"
    )
...
# If no apps has been added to the repository, move the .Acora folder to the project folder (Convert to multi-project repository)
    ...
    if ($project -eq ".") {
        ...
        if (Test-Path $ALGoSettingsFile) {
            ...
            if ($appCount -eq 0) {
                ...
                throw "Repository is setup for a single project, cannot add a project. Move all appFolders, testFolders and the .Acora folder to a subdirectory in order to convert to a multi-project repository."
            }
        }
        else {
...
function GetProjectsFromRepository {
    ...
    if ($projectsFromSettings) {
        ...
        # For multiple projects, get all folders in two levels below the base folder containing an .Acora folder with a settings.json file
        $projects = @(Get-ChildItem -Path $baseFolder -Recurse -Depth 2 -Force | Where-Object { $_.PSIsContainer -and (Test-Path (Join-Path $_.FullName ".Acora/settings.json") -PathType Leaf) } | ForEach-Object { $_.FullName.Substring($baseFolder.length+1) })
        # To support single project repositories, we check for the .Acora folder in the root
        if (Test-Path (Join-Path $baseFolder ".Acora/settings.json") -PathType Leaf) {
            $projects += @(".")
        }
    }
